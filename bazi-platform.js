...
function train(rows,epochs=420,lr=.18){const w=new Array(FEATURES.length).fill(0);let b=0;for(let e=0;e<epochs;e++){const gw=w.map(()=>0);let gb=0;for(const row of rows){let z=b;w.forEach((v,i)=>z+=v*row.x[i]);const err=sigmoid(z)-row.y;gw.forEach((_,i)=>gw[i]+=err*row.x[i]);gb+=err}w.forEach((_,i)=>w[i]-=lr*gw[i]/rows.length);b-=lr*gb/rows.length}return{weights:w,bias:b,version:MODEL_VERSION,samples:rows.length,artifactClass:'runtime-trained-demo-fallback'}}
function frozenModel(){const f=window.BAZI_FROZEN_MODEL;if(!f)return null;return{weights:[...f.weights],bias:Number(f.bias),version:f.version,samples:0,artifactClass:f.artifactClass||'frozen-reference',predictionHorizonDays:f.predictionHorizonDays||7,featureSchemaVersion:f.featureSchemaVersion||'unknown'}}
...
function renderTrajectory(f,r){
 ...
 const pts=vals.map((v,i)=>`${18+i*(264/Math.max(vals.length-1,1))},${108-(v/100)*82}`).join(' ');
 const last=vals.length?vals[vals.length-1]:baseline;
 const nextAtBaseline={status:'completed',duration_minutes:Number(p?.baseline_duration||25),fatigue:'low',difficulty:'appropriate',engagement:baseline};
 const resumedRisk=risk(computeFeatures(p,[...events(),nextAtBaseline]));
 const delta=Math.round(last-baseline);
 const signal=delta<=-15?'meaningfully below':delta<=-5?'below':'near';
 $('#trajectoryPanel').innerHTML=`<div class="mini-grid">
  <div style="grid-column:span 2"><div class="notice"><strong>${esc(p.display_name)} is ${signal} their own baseline.</strong><br>Latest observed engagement: ${last} · personal baseline: ${baseline} · change: ${delta>=0?'+':''}${delta} points.</div></div>
  <div class="mini-stat"><strong>${r}%</strong><span>Current signal</span></div>
  <div class="mini-stat"><strong>${resumedRisk}%</strong><span>Illustrative signal after one baseline-level completion</span></div>
 </div>
 <div style="margin-top:14px;overflow:auto"><svg viewBox="0 0 300 126" role="img" aria-label="Recent engagement trajectory" style="width:100%;max-width:470px;height:142px;background:#f7fbf9;border:1px solid #d8e8e2;border-radius:10px"><line x1="18" y1="${108-(baseline/100)*82}" x2="282" y2="${108-(baseline/100)*82}" stroke="#7fa99b" stroke-dasharray="4 4"/><polyline points="${pts}" fill="none" stroke="#bf5b50" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>${vals.map((v,i)=>`<circle cx="${18+i*(264/Math.max(vals.length-1,1))}" cy="${108-(v/100)*82}" r="4.5" fill="#bf5b50"/>`).join('')}<text x="18" y="121" fill="#60736b" font-size="10">recent observed events</text><text x="184" y="${102-(baseline/100)*82}" fill="#60736b" font-size="10">personal baseline</text></svg></div>`;
}
...
async function boot(){model=frozenModel()||train(syntheticRows());const ready=await Store.init();wire();if(Store.mode==='local'){$('#authGate').classList.add('hidden');render()}else if(ready){$('#authGate').classList.add('hidden');render()}else{$('#authGate').classList.remove('hidden')} }
